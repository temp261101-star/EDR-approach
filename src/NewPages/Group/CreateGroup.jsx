...
export default function CreateGroup() {
  ...
  useEffect(() => {
    if (location.state) {
      const { isManageMode, groupId, groupName, groupDescription } =
        location.state;
  ...
  const handleUpdateGroup = async () => {
    ...
    try {
      ...
      devices.forEach((device) => {
        console.log(
          `Device: ${device.name}, ID: ${
            device.id
          }, Type: ${typeof device.id}, String: "${String(device.id)}"`
        );
      });
      ...
      const devicesToRemove = existingGroupDevices.filter((device) => {
        ...
        console.log(
          `Comparing: deviceOriginalId="${deviceOriginalId}" vs selectedDeviceIds=`,
          selectedDeviceIds
        );
      ...
      console.log(
        "Devices to remove with full details:",
        devicesToRemove.map((d) => ({
          relationshipId: d.id,
          originalDeviceId: d.originalDeviceId,
          name: d.name,
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 p-6 space-y-6">
      ...
      <div className="bg-gray-900 rounded-2xl p-5 mx-8 shadow">
        ...
        <div className="grid grid-cols-3 gap-4 items-center mb-4">
          {/* search  */}
          <div className="grid-cols-1 flex justify-center items-center bg-gray-800 border border-gray-700 rounded-lg px-2 focus-within:ring-2 focus-within:ring-blue-600 focus-within:border-transparent transition-all flex-1 min-w-[250px]">
...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 mb-5">
  {/* Filter category - spans 2 columns on lg, full width on smaller screens */}
  ...
  {filterCategory && (
    <>
      {/* Filter value - spans 2 columns on lg, full width on smaller screens */}
      <div className="relative lg:col-span-2 min-w-0 animate-fadeIn">
        <select
          value={filterValue}
      ...
      {/* Clear button - spans 2 columns on lg, full width on smaller screens */}
      <div className="lg:col-span-1 min-w-0">
        <button
          onClick={clearFilters}
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-red-600/20 hover:bg-red-600/30 border border-red-600/40 text-red-400 rounded-lg transition-all text-sm font-medium hover:scale-105 active:scale-95 whitespace-nowrap"
        >
            ...
            <tbody>
              {filteredDevices.length > 0 ? (
                filteredDevices.map((d) => {
                  ...
                  console.log(
                    `Device ${d.name} (${currentDeviceId}): inGroup=${isCurrentlyInGroup}, selected=${isSelected}`
                  );
        ...
        <div className="flex items-center justify-between mt-3">
          <p className="text-sm text-gray-400">
            ...
            {isManageMode &&
              ` (${existingGroupDevices.length} currently in group)`}
          </p>
