...
export default function Table({
    ...
    if (data.length === 0 && endpoint) {
      const fetchData = async () => {
        ...
        try {
          ...
          console.log("table xyz:", response);
  ...
  }, [endpoint, dataPath]); // Changed: removed data.length, added endpoint and dataPath

  // Add this useEffect to reset data when endpoint changes
  useEffect(() => {
    if (endpoint) {
      setApiData([]);
      setCurrentPage(1);
  ...
  const displayColumns = useMemo(() => {
    ...
    if (showActions && actions.length > 0) {
      cols.push("actions"); // Add actions as last column
    }
    return cols;
  }, [actualColumns, showActions, actions]);
      ...
      <div className="relative bg-gradient-to-r  px-6 py-4 border-b border-gray-600">
        ...
        <div className="relative flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
          <div className="flex items-center gap-3">
            <h2 className="text-l font-bold text-white tracking-wide">
        ...
        <table className="w-full border-collapse table-auto min-w-full ">
          ...
          <tbody className="bg-gray-900 divide-y divide-gray-700">
            {paginatedData.map((row, i) => (
              ...
              >
                ...
                {displayColumns.map((col) => (
                  <td
                    key={col}
                    className={`py-2 px-4 text-gray-200 text-sm ${
            ...
            {isLoading && (
              <tr>
                <td
                  colSpan={displayColumns.length + (showCheckboxes ? 1 : 0)}
